    ...
    switch (channel.toLowerCase()) {
      case 'markel':
        try {
          ...
          // Criar entrada na tabela quote_items para Markel
          const { error: quoteItemError } = await supabaseAdmin
            .from('quote_items')
            .insert({
              quote_id,
              insurer: 'markel',
              ...
              currency: 'USD',
              payload: result,
              submission_id: result.submissionNumber,
              response_status: 'success'
            })

          if (quoteItemError) {
            console.error('Error creating Markel quote item:', quoteItemError)
          } else {
            console.log('✅ Markel quote item created successfully')
          }
          ...
          // Criar entrada na tabela quote_items mesmo para falha (para auditoria)
          await supabaseAdmin
            .from('quote_items')
            .insert({
              quote_id,
      ...
      case 'cf':
        try {
          ...
          submission_data = {
            ...
            destination: 'cf',
            status: 'submitted',
            external_id: result.cnfQuoteNumber,
            request_data: quote,
            response_data: result,
            submitted_at: new Date().toISOString()
          }

          // Extrair premium da resposta da C&F
          const cfPremium = result.cnfPolicyService?.cnfPolicyData?.data?.policy?.PolicyPremium || null
          const cfQuoteNumber = result.cnfQuoteNumber

          // Criar entrada na tabela quote_items para C&F
          const { error: quoteItemError } = await supabaseAdmin
            .from('quote_items')
            .insert({
              quote_id,
              insurer: 'cf',
              channel: 'api',
              premium: cfPremium ? parseFloat(cfPremium) : null,
              currency: 'USD',
              payload: result,
              submission_id: cfQuoteNumber,
              response_status: 'success'
            })

          if (quoteItemError) {
            console.error('Error creating C&F quote item:', quoteItemError)
          } else {
            console.log('✅ C&F quote item created successfully')
          }
